...
#### FR-I18N: Интернационализация интерфейса

**Описание:** Поддержка нескольких языков интерфейса с автоматическим определением языка пользователя.

**Требования:**
- Автоматическое определение языка на основе настроек VS Code
- Поддержка русского и английского языков
- Локализация всех пользовательских сообщений
- Функция `t()` для получения переводов с поддержкой параметров
- Согласованность терминологии во всех сообщениях
...
**Области локализации:**
- Сообщения об ошибках в UI (showErrorMessage)
- Информационные сообщения (showInformationMessage)
- Подсказки и инструкции
- Заголовки диалогов и окон
- Текст кнопок и меню

**Acceptance Criteria:**
- [ ] Функция `t()` определяет язык автоматически
- [ ] Все сообщения об ошибках переведены
- [ ] Все информационные сообщения переведены
- [ ] Параметры в сообщениях подставляются корректно
- [ ] Интерфейс переключается при изменении языка в VS Code
- [ ] Словари содержат все необходимые сообщения
- [ ] Нет hardcoded текста в английском языке в коде
...
// Вывод: "Failed to connect to CodeLab. Please check your connection."
// или на русском: "Не удалось подключиться к CodeLab. Проверьте соединение."
...
// Вывод: "API error (401): Unauthorized"
// или на русском: "Ошибка API (401): Unauthorized"

// Обработка ошибки валидации
vscode.window.showErrorMessage(
  t('errors.validationError', { 
    message: error.zodError.errors[0].message 
  })
);
```
